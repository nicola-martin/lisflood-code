...
from .modflow6 import ModFlowSimulation
...
def reshape1(map):
    """
    Decompressing Lisflood maps from 1D to 2D with missing values
...
def decompress1(map, nanvalue=None):
    """
...
class groundwater_modflow(HydroModule):

    def __init__(self, modflow_variable):
        self.var = modflow_variable
    ...
    **Functions**
    ...
    def lisflood2modflow(self, variable, correct_boundary=False):
        # Converting flow [L/T] from 2D Lisflood map to 2D ModFlow map
        if correct_boundary:
        ...
        array = (np.bincount(
            ...
            variable.ravel()[self.indices['lisflood_index']] * self.indices['area'],
    ...
    def modflow2lisflood(self, variable, correct_boundary=False):
        # Converting flow [L/T] from 2D ModFLow map to 2D Lisflood map
        maskinfo = MaskInfo.instance()
        ...
        if correct_boundary:
            lisflood_cell_area = self.corrected_lisflood_cell_area.ravel()
        else:
            lisflood_cell_area = self.var.PixelArea.ravel()  # in m2

        array = (np.bincount(
            self.indices['lisflood_index'],
            weights = variable_copy.ravel()[self.indices['ModFlow_index']] * self.indices['area'],
               minlength = maskinfo.info.shape[0] * maskinfo.info.shape[1]) / decompress1(lisflood_cell_area, nanvalue=0)).reshape(
               maskinfo.info.shape).astype(variable_copy.dtype)

        #dmap = maskinfo.info.maskall.copy()
        #array[dmap.mask] = np.nan
        ####array[maskinfo['mask'] == 1] = np.nan

        return array

    def modflow2lisfloodbis(self, variable, correct_boundary=False):
        """
        Converting the 2D ModFLow capillary rise map into the fraction of area where capillary rise occurs
        in the 2D Lisflood maps. return a fraction for each Lisflood cell, input is the ModFlow capillary rise map
        ...
        array = (np.bincount(
            self.indices['lisflood_index'],
            weights=variable_copy.ravel()[self.indices['ModFlow_index']] * self.indices['area'],
                minlength=maskinfo.info.shape[0] * maskinfo.info.shape[1])
                / decompress1(lisflood_cell_area, nanvalue=0)).reshape(maskinfo.info.shape).astype(variable_copy.dtype)
        ###array[maskinfo['mask'] == 1] = np.nan
        #dmap = maskinfo.info.maskall.copy()
        #array[dmap.mask] = np.nan
        return array
    ...
    def initial(self):
        ...
        modflow_directory = binding['PathGroundwaterModflow']
        modflow_directory_output = binding['PathGroundwaterModflowOutput']
        directory_mf6dll = binding['path_mf6dll']
        if not (os.path.isdir(directory_mf6dll)):
            msg = "Error 222: Path to Modflow6 files does not exists "
            #raise lisfloodDirError(directory_mf6dll, msg, sname='path_mf6dll')
            print (msg,directory_mf6dll )
        nlay = int(loadmap('nlay'))

        rasterio = importlib.import_module("rasterio", package=None)
        with rasterio.open(binding['modflow_basin'], 'r') as src:
        ...
        with rasterio.open(binding['topo_modflow'], 'r') as src:
            topography = src.read(1).astype(np.float32)
            topography[modflow_basin == False] = np.nan

        with rasterio.open(binding['chanRatio'], 'r') as src:
            ...
            self.var.channel_ratio = np.where(self.var.channel_ratio > 0,
                 ...
                 np.where(self.var.channel_ratio + factor_channelratio < 0, 0, self.var.channel_ratio + factor_channelratio)), 0)

        # uploading arrays allowing to transform 2D arrays from ModFlow to Lisflood and conversely
        modflow_x = np.load(os.path.join(binding['lisflood_modflow_indices'], 'modflow_x.npy'))
        modflow_y = np.load(os.path.join(binding['lisflood_modflow_indices'], 'modflow_y.npy'))
        lisflood_x = np.load(os.path.join(binding['lisflood_modflow_indices'], 'lisflood_x.npy'))
        lisflood_y = np.load(os.path.join(binding['lisflood_modflow_indices'], 'lisflood_y.npy'))

        self.indices = {
            'area': np.load(os.path.join(binding['lisflood_modflow_indices'], 'area.npy')),
            'ModFlow_index': np.array(modflow_y * self.domain['ncol'] + modflow_x),
            'lisflood_index': np.array(lisflood_y * maskinfo.info.shape[1] + lisflood_x)
        }

        #indices_cell_area = np.bincount(self.indices['lisflood_index'], weights=self.indices['area'],
        #                                minlength=maskinfo['mapC'][0])
        indices_cell_area = np.bincount(self.indices['lisflood_index'], weights=self.indices['area'], minlength=mapC[0])

        area_correction = (decompress1(self.var.PixelArea, nanvalue=0) / indices_cell_area)[self.indices['lisflood_index']]
        self.indices['area'] = self.indices['area'] * area_correction

        permeability_m_s = binding['permeability']
        if is_float(permeability_m_s):  # aquifer permeability is constant
        ...
        else:  # aquifer permeability is given as a tif file at lisflood resolution
            perm1 = loadmap('permeability') + maskinfo.in_zero()
            perm2 = maskinfo.info.maskall.copy()
            perm2[~maskinfo.info.maskflat] = perm1[:]
            # Lisflood 2D array is converted to Modflow 2D array
            ...
            for i in range(len(modflow_x)):
                v = p1[lisflood_y[i], lisflood_x[i]]
        ...
        self.porosity = binding['poro']  # default = 0.1
        if is_float(self.porosity):  # aquifer porosity is constant
            ...
            self.porosity = np.full((nlay, self.domain['nrow'], self.domain['ncol']), self.porosity,
                                    dtype=np.float32)
        else:  # aquifer porosity is given as a tif file at lisflood resolution
            poro = loadmap('poro')
            #p1 = maskinfo['maskall'].copy()
        ...
        self.thickness = binding['thickness']  # default = 0.1
        ...
        # Converting the lisflood soil thickness into ModFlow map, then soil thickness will be removed from
        ...
        if 'use_soildepth_as_GWtop' in binding:
            ...
            if binding['use_soildepth_as_GWtop']=="True":
                soildepth_as_GWtop = True
        ...
        if 'correct_soildepth_underlakes' in binding:
            ...
            if binding['correct_soildepth_underlakes']=="True":
                correct_depth_underlakes = True
        ...
        if soildepth_as_GWtop:  # topographic minus soil depth map is used as groundwater upper boundary
            ...
            if correct_depth_underlakes:
                ...
                soildepth_modflow = self.lisflood2modflow(decompress1(soil_depth_temp)) + 0.05
                ...
                soildepth_modflow = self.lisflood2modflow(decompress1(self.var.SoilDepth2[0])) + 0.05
                soildepth_modflow[np.isnan(soildepth_modflow)] = 0
            ...
            if correct_depth_underlakes:  # we make a manual correction
                ...
                soildepth_modflow = self.lisflood2modflow(decompress1(soil_depth_temp))
                soildepth_modflow[np.isnan(soildepth_modflow)] = 0
            else:
                if verboseGW:
        ...
        if not correct_depth_underlakes:  # we make a manual correction
            #waterBodyID_temp = loadmap('waterBodyID').astype(np.int64)
            waterBodyID_temp = loadmap('LakeMask')
        lake_modf = np.where(waterBodyID_temp != 0, 1, 0)
        help = decompressNp1(lake_modf)
        lake_modf = self.lisflood2modflow(help)
        soildepth_modflow[np.isnan(lake_modf)] = 0
        self.layer_boundaries[0] = np.where(lake_modf <= 0, np.where(self.var.channel_ratio > 0,
              self.layer_boundaries[0] - 1,self.layer_boundaries[0]),self.layer_boundaries[0])
        ...
        if 'load_init_water_table' in binding:
            ...
            if binding['load_init_water_table']=="True":
                self.var.load_init_water_table = True
        ...
        if self.var.load_init_water_table:
            ...
            # initiate head 2 m below the top of the aquifer layer
        ...
        if 'Groundwater_pumping' in binding:
            ...
            if binding['Groundwater_pumping']=="True":
                self.var.GW_pumping = True
        ...
        if 'use_complex_solver_for_modflow' in option:
            if binding['use_complex_solver_for_modflow']=="True":
                self.var.use_complex_solver_for_modflow = True

        self.var.use_super_complex_solver_for_modflow = False
        if 'use_super_complex_solver_for_modflow' in binding:
            if binding['use_super_complex_solver_for_modflow']=="True":
                self.var.use_super_complex_solver_for_modflow = True

        self.var.availableGWStorageFraction = 0.85
        ndays = settings.model_steps[1]
        # nadys= globals.dateVar['intEnd']

        load_modflow_disk = False
        if 'load_modflow_from_disk' in binding:
            if binding['load_modflow_from_disk']=="True":
                load_modflow_disk = True
        ...
        if self.var.GW_pumping:
            ...
            # for Burgenland we set up only one well for each lisflood cell (1*1km), thus only one well every ten ModFlow cells
            # TODO NumOfModflowWellsInlisfloodCell = 1 #Must be even or 1
            ...
            if 'water_table_limit_for_pumping' in binding:
                ...
                self.var.availableGWStorageFraction = np.maximum(
                    np.minimum(0.98, loadmap('water_table_limit_for_pumping')),0)
                # if 85% of the ModFlow cell is empty, we prevent pumping in this cell
            if verboseGW:
            ...
            self.modflow = ModFlowSimulation(
                ...
                load_from_disk=load_modflow_disk,
                setpumpings=True,
            ...
            self.modflow = ModFlowSimulation(
                ...
                timestep=self.var.modflow_timestep,
                specific_storage=0,
                specific_yield=float(binding['poro']),
                nlay=nlay,
                nrow=self.domain['nrow'],
                ncol=self.domain['ncol'],
                rowsize=self.domain['rowsize'],
        ...
        # initializing arrays
        self.var.capillar = maskinfo.in_zero()
        self.var.baseflow = maskinfo.in_zero()
        self.var.depth = maskinfo.in_zero()
        ...
        self.var.sumGwRecharge = maskinfo.in_zero()
        self.var.capriseindex = maskinfo.in_zero()
        self.var.modflow_compteur = 0

        self.var.modflow_watertable = np.copy(head)  # water table will be also saved at modflow resolution

        # initial water table map is converting into lisflood map
        self.var.head = compressArray(self.modflow2lisflood(head),pcr=False)

        self.var.writeerror = False
        if 'writeModflowError' in binding:
            if binding['writeModflowError']=="True":
                self.var.writeerror = True


        if self.var.writeerror:
            # This one is to check model's water balance between ModFlow and lisflood exchanges ModFlow discrepancy
        ...
        # converting the groundwater storage from ModFlow to lisflood map (in meter)
        self.var.groundwater_storage_available = compressArray(
            self.modflow2lisflood(self.var.groundwater_storage_top_layer),pcr=False)  # used in water demand module then
        self.var.gwstorage_full = compressArray(self.modflow2lisflood(
            (self.layer_boundaries[0] - self.layer_boundaries[1]) * self.porosity[
                0]),pcr=False)  # use in water damnd to limit pumping

        # permeability need to be translated into lisflood map to compute leakage from surface water bodies
        self.var.permeability = compressArray(self.modflow2lisflood(self.permeability[0]),pcr=False) * self.coefficient
        ii = 1
    ...
    def dynamic(self):
        ...
        self.var.modfPumpingM_actual = maskinfo.in_zero() # compressArray(self.modflow2lisflood(self.permeability[0]*0))
        step = self.var.currentTimeStep() - self.var.firstTimeStep()

        if self.var.currentTimeStep() == 1 or (step % self.var.modflow_timestep) == 0:
        #if self.var.modflow_timestep == 1 or ((dateVar['curr'] - int(dateVar['curr'] / self.var.modflow_timestep)
        #                                * self.var.modflow_timestep) == 1):  # if it is the first step of the week,months...
            ...
            self.var.sumGwRecharge = 0

        # Adding recharge of the day to the weekly (or bi-weekly, or monthly...) sum
        self.var.sumGwRecharge = self.var.sumGwRecharge + self.var.GwPercUZLZPixel
        ...
        ##if dateVar['curr'] == 1 or (dateVar['curr'] % self.var.modflow_timestep) == 0:
            ...
            # converting the lisflood recharge into ModFlow recharge (in meter)
            # we avoid recharge on saturated ModFlow cells, thus lisflood recharge is concentrated  on unsaturated cells
            corrected_recharge = np.where(self.var.capriseindex == 1, self.var.sumGwRecharge * 0, self.var.sumGwRecharge / (1 - self.var.capriseindex))
            groundwater_recharge_modflow = self.lisflood2modflow(decompress1(corrected_recharge, nanvalue=0), correct_boundary=False)
            ...
            """
            if self.var.GW_pumping:
                ## Groundwater demand from lisflood installs wells in each Modflow cell

                # Groundwater pumping demand from the lisflood waterdemand module, will be decompressed to 2D array
                # lisflood 2D groundwater pumping array is converted into Modflow 2D array
                # Pumping is given to ModFlow in m3 and < 0
                # print('mean modflow pumping [m]: ', np.nanmean(self.var.modfPumpingM))
                groundwater_abstraction = - self.lisflood2modflow(decompress(self.var.modfPumpingM)) * domain['rowsize'] * \
                                          domain['colsize']
                # give the information to ModFlow
            ...
            self.var.groundwater_storage_top_layer = (np.where(head > self.layer_boundaries[0],
                     self.layer_boundaries[0], head) - self.layer_boundaries[1]) * self.porosity[0]
            # converting the groundwater storage from ModFlow to lisflood map (in meter)
            self.var.groundwater_storage_available = compressArray(self.modflow2lisflood(self.var.groundwater_storage_top_layer),pcr=False)
            ...
            if self.var.writeerror:
                ...
                if self.var.GW_pumping:
                    ...
                    self.var.modfPumpingM_actual = - compressArray(
                        self.modflow2lisflood(actual_pumping_modflow_array) / (domain['rowsize'] * domain['colsize']))
                ...
                # to check lisflood-ModFlow projection is working well
            ...
            # converting flows from ModFlow to lisflood domain
            self.var.capillar = compressArray(self.modflow2lisflood(capillar),pcr=False)
            baseflow = baseflow.astype('float64')  # required for runoff concentration (lib2.runoffConc)
            self.var.baseflow = compressArray(self.modflow2lisflood(baseflow),pcr=False)

            # computing saturated fraction of each lisflood cells (where water table >= soil bottom)
            self.var.capriseindex = compressArray(self.modflow2lisfloodbis(groundwater_outflow2),pcr=False)
            # initialized in landcoverType module, self.var.capriseindex is the fraction of saturated ModFlow cells in each lisflood cell

            # head = self.modflow.decompress(self.modflow.head.astype(np.float32))
            # updating water table maps both at lisflood and ModFlow resolution
            self.var.head = compressArray(self.modflow2lisflood(head),pcr=False)
            self.var.gwdepth = compressArray(self.modflow2lisflood(self.layer_boundaries[0]),pcr=False) - self.var.head
            ...
            if 'gw_depth_sim_obs' in binding:
                ...
                head_adjusted = self.layer_boundaries[0] - self.lisflood2modflow(decompress1(self.var.gwdepth_adjusted))
            ...
            self.var.modfPumpingM = maskinfo.in_zero()

            # to check lisflood-ModFlow projection is working well
            # if self.var.writeerror:
            #    # computing the total recharge rate provided by lisflood for this time step (in m3/timestep)
            #    total_recharge_m3_lisflood = (self.var.sumed_sum_gwRecharge * self.var.cellArea).sum()  # m3/timestep, for water balance computation
            #
            #    print('ModFlow-lisflood input conversion error : ', np.round(100 * (total_recharge_m3_lisflood - sumModFlowin)/sumModFlowin, 2), ' %')
            #    if self.var.GW_pumping:
            #        print('ModFlow-lisflood output conversion error : ', np.round(100 * (np.nansum((self.var.capillar+self.var.baseflow) * self.var.modflow_timestep *
            #                                                                                    self.var.cellArea) - np.nansum(groundwater_abstraction) - sumModFlowout) / sumModFlowout, 2), ' %')
            #        # Check crossed models:
            #        print('ModFlow discrepancy crossed: ', np.round(100 * (total_recharge_m3_lisflood -
            ...
            if self.var.currentTimeStep()  ==  settings.model_steps[1]:
                if self.var.writeerror:
                    discrep_filename = binding['PathGroundwaterModflowOutput'] + '/' + 'ModFlow_DiscrepancyError.txt'
                    discrep_file = open(discrep_filename, "w")
                    sum_modflow_errors = 0
                    threeshold_modflow_error = 0.01  # in percentage

                    for tt in range(settings.model_steps[1]):
