...
from ..groundwater_modflow.modflow6 import ModFlowSimulation
...
class groundwater_modflow:
    ...
    **Functions**
    ...
    def modflow2LISFOOD(self, variable, correct_boundary=False):
        ...
        if correct_boundary:
            ...
            lisflood_cell_area = self.var.cellArea.ravel()  # in m2

        array = (np.bincount(
            self.indices['lisflood_index'],
            weights=variable_copy.ravel()[self.indices['ModFlow_index']] * self.indices['area'],
            minlength=maskinfo['shape'][0] * maskinfo['shape'][1]) / decompress(lisflood_cell_area, nanvalue=0)).reshape(
    ...
    def modflow2LISFLOODbis(self, variable, correct_boundary=False):
        ...
        ) / decompress(lisflood_cell_area, nanvalue=0)).reshape(maskinfo['shape']).astype(variable_copy.dtype)
        array[maskinfo['mask'] == 1] = np.nan
        return array
    ...
    def initial(self):
        ...
        if self.var.modflow:
            ...
            modflow_x = np.load(os.path.join(cbinding('lisflood_modflow_indices'), 'modflow_x.npy'))
            modflow_y = np.load(os.path.join(cbinding('lisflood_modflow_indices'), 'modflow_y.npy'))
            lisflood_x = np.load(os.path.join(cbinding('lisflood_modflow_indices'), 'lisflood_x.npy'))
            lisflood_y = np.load(os.path.join(cbinding('lisflood_modflow_indices'), 'lisflood_y.npy'))

            self.indices = {
                'area': np.load(os.path.join(cbinding('lisflood_modflow_indices'), 'area.npy')),
                'ModFlow_index': np.array(modflow_y * self.domain['ncol'] + modflow_x),
                'lisflood_index': np.array(lisflood_y * maskinfo['shape'][1] + lisflood_x)
            }
            ...
            if soildepth_as_GWtop:  # topographic minus soil depth map is used as groundwater upper boundary
                ...
                if correct_depth_underlakes:
                    ...
                    soildepth_modflow = self.lisflood2modflow(decompress(soil_depth_temp)) + 0.05
                    ...
                    soildepth_modflow = self.lisflood2modflow(decompress(self.var.soildepth12)) + 0.05
                    soildepth_modflow[np.isnan(soildepth_modflow)] = 0
                ...
                if correct_depth_underlakes:  # we make a manual correction
                    ...
                    soildepth_modflow = self.lisflood2modflow(decompress(soil_depth_temp))
                    soildepth_modflow[np.isnan(soildepth_modflow)] = 0
                else:
                    if verboseGW:
            ...
            lake_modf = self.lisflood2modflow(decompress(lake_modf))
            ...
            self.var.head = compressArray(self.modflow2lisflood(head))
            ...
            self.var.groundwater_storage_available = compressArray(
                self.modflow2lisflood(self.var.groundwater_storage_top_layer))  # used in water demand module then
            ...
            self.var.permeability = compressArray(self.modflow2lisflood(self.permeability[0])) * self.coefficient
    ...
    def dynamic(self):
        ...
        self.var.modfPumpingM_actual = globals.inZero.copy()  # compressArray(self.modflow2lisflood(self.permeability[0]*0))
        ...
        if dateVar['curr'] == 1 or (dateVar['curr'] % self.var.modflow_timestep) == 0:
            ...
            groundwater_recharge_modflow = self.lisflood2modflow(decompress(corrected_recharge, nanvalue=0),
            ...
            # converting the groundwater storage from ModFlow to lisflood map (in meter)
            self.var.groundwater_storage_available = compressArray(
                self.modflow2lisflood(self.var.groundwater_storage_top_layer))
            ...
            self.var.capillar = compressArray(self.modflow2lisflood(capillar))
            baseflow = baseflow.astype('float64')  # required for runoff concentration (lib2.runoffConc)
            self.var.baseflow = compressArray(self.modflow2lisflood(baseflow))

            # computing saturated fraction of each lisflood cells (where water table >= soil bottom)
            self.var.capriseindex = compressArray(self.modflow2lisfloodbis(
                groundwater_outflow2))  # initialized in landcoverType module, self.var.capriseindex is the fraction of saturated ModFlow cells in each lisflood cell

            # head = self.modflow.decompress(self.modflow.head.astype(np.float32))
            # updating water table maps both at lisflood and ModFlow resolution
            self.var.head = compressArray(self.modflow2lisflood(head))
            self.var.gwdepth = compressArray(self.modflow2lisflood(self.layer_boundaries[0])) - self.var.head
            ...
            if 'gw_depth_sim_obs' in binding:
                ...
                head_adjusted = self.layer_boundaries[0] - self.lisflood2modflow(decompress(self.var.gwdepth_adjusted))
